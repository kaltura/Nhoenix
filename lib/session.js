...
new types.ExceptionType('INVALID_KS_FORMAT', 'Invalid KS format');
new types.ExceptionType('KS_EXPIRED', 'KS expired');
new types.ExceptionType('PARTNER_INVALID', 'Partner invalid');
...
function getSecrets(partnerId) {
    ...
    throw new types.KalturaAPIException(ExceptionType.PARTNER_INVALID);
}

function validate(session) {
    const now = Date.now() / 1000;
    if(session.expiry < now) {
        throw new types.KalturaAPIException(ExceptionType.KS_EXPIRED);
    }
}
...
module.exports = {
    ...
    parse: ks => {
        ...
        if(version != 'v2') {
            throw new types.KalturaAPIException(ExceptionType.INVALID_KS_FORMAT);
        }
