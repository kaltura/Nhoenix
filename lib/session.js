...
new types.KalturaExceptionType('INVALID_KS_FORMAT', 'Invalid KS format');
new types.KalturaExceptionType('KS_EXPIRED', 'KS expired');
new types.KalturaExceptionType('PARTNER_INVALID', 'Partner invalid');
...
function getSecrets(partnerId) {
    ...
    throw new types.KalturaAPIException(KalturaExceptionType.PARTNER_INVALID);
}

function validate(session) {
    const now = Date.now() / 1000;
    if(session.expiry < now) {
        throw new types.KalturaAPIException(KalturaExceptionType.KS_EXPIRED);
    }
}
...
module.exports = {
    ...
    parse: ks => {
        ...
        if(version != 'v2') {
            throw new types.KalturaAPIException(KalturaExceptionType.INVALID_KS_FORMAT);
        }
