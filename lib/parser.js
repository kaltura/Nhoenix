...
function validateProperty(type, property, validObjects) {
    if(!property.name.match(/^[a-z][a-zA-Z0-9]*$/)) {
        throw `Type [${type.name}] property [${property.name}] name is invalid`;
    }
    if(!property.description.match(/^[A-Z].+/)) {
        throw `Type [${type.name}] property [${property.name}] description is invalid`;
    }
    if(types.indexOf(property.type) < 0 && !validObjects[property.type]) {
        throw `Type [${type.name}] property [${property.name}] invalid type [${property.type}]`;
    }
}
