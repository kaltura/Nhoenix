...
function validateArgument(service, action, arg) {
    if(!arg.name) {
...
function validateAction(service, action) {
    ...
    if(action.args) {
        action.args.forEach(arg => validateArgument(service, action.name, arg));
    }
}
...
function validateService(service) {
    ...
    Object.keys(service.actions).forEach(action => validateAction(service.name, service.actions[action]));
}
...
function parseController(doc) {
    ...
    var service = {
        ...
        actions: doc
            .filter(comment => comment.memberof === controller.longname && comment.action)
            .reduce((data, comment) => {
                if(comment.kind != 'function') {
                    throw `Action [${controller.service}.${comment.action}] is not a function`;
                }

                if(data[comment.action]) {
                    throw `Action [${controller.service}.${comment.action}] appears more than once`;
                }

                var action = {
                    name: comment.action,
                    description: comment.description,
                    method: controllerModule[comment.name]
                };
                ...
                if(action.method.length) {
                    if(!comment.params) {
                        throw `Action [${controller.service}.${comment.action}] arguments not described`;
                    }
                    var args = false;
                    ...
                    if(!args) {
                        throw `Action [${controller.service}.${comment.action}] failed to parse argument names`;
                    }
                    ...
                    action.args = comment.params.map(param => {
                        ...
                        if(index < 0) {
                            throw `Action [${controller.service}.${comment.action}] argument [${param.name}] does not exist`;
                        }
                        if(!param.type || !param.type.names || !param.type.names.length) {
                            throw `Action [${controller.service}.${comment.action}] argument [${param.name}] type not defined`;
                        }
                        if(!param.description || !param.description.length) {
                            throw `Action [${controller.service}.${comment.action}] argument [${param.name}] description not defined`;
                        }
                        ...
                        if(tags.length > 1) {
                            tags.slice(1).forEach(tag => {
                                ...
                                if(tag.startsWith('min') || tag.startsWith('max')) {
                                    if(isNaN(value)) {
                                        throw `Action [${controller.service}.${comment.action}] argument [${param.name}] ${tag} is not a number`;
                                    }
                    ...
                    if(args.length > action.args.length) {
                        ...
                        throw `Action [${controller.service}.${comment.action}] missing arguments [${notFound}] definitions`;
                    }
                }
